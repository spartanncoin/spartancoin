...
uint256 hashGenesisBlock("0xef2e6028090dbae7e0672f49bec49bc685fac2d71830730fc32b6d96c4296a02");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Spartancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Spartancoin Signed Message:\n";
...
//rewards
int64 nLastRandomBlock = 1;
int64 nCurrentRandom = 1;
bool calculateRandomBlock = true;
...
{
    // Spartancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Spartancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Spartancoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xef2e6028090dbae7e0672f49bec49bc685fac2d71830730fc32b6d96c4296a02");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x842c7513b6410bfe27ef309ae9d4326e2a3e4c29a78fb2c9336ddd0d1da511da"));
        ...
        if (false && block.GetHash() != hashGenesisBlock)
        {
            ...
            while(true)
            {
                ...
                {
                    printf("nonce %08X: hash = %s (target = %s)\n", block.nNonce, trash.ToString().c_str(), hashTarget.ToString().c_str());
                }
                ++block.nNonce;
                if (block.nNonce == 0)
                {
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Spartancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SpartancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpartancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SpartancoinMiner(CWallet *pwallet)
{
    printf("SpartancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spartancoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running SpartancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SpartancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpartancoinMiner, pwallet));
}
